...
class HeacModel(torch.nn.Module):
    ...
    def __init__(self, categorical_field_dims, numerical_num, embed_dim, bottom_mlp_dims, tower_mlp_dims, task_num, expert_num, critic_num, dropout):
        ...
        self.critic = torch.nn.ModuleList([MultiLayerPerceptron(
            bottom_mlp_dims[-1], tower_mlp_dims, dropout) for i in range(critic_num)])
        self.expert_gate = torch.nn.Sequential(torch.nn.Linear(
            embed_dim * 2, expert_num), torch.nn.Softmax(dim=1))
        self.critic_gate = torch.nn.Sequential(torch.nn.Linear(
            embed_dim * 2, critic_num), torch.nn.Softmax(dim=1))
    ...
    def forward(self, categorical_x, numerical_x):
        ...
        for i in range(self.task_num):
            idxs = torch.tensor([i for j in range(batch_size)]
                                ).view(-1, 1).to(categorical_x.device)
            task_emb = self.task_embedding(idxs).squeeze(1)
            gate_input_emb.append(
                torch.cat([task_emb, torch.mean(emb, dim=1)], dim=1).view(batch_size, -1))

        emb = emb.view(-1, self.embed_output_dim)

        expert_gate_value = [self.expert_gate(
            gate_input_emb[i]).unsqueeze(1) for i in range(self.task_num)]
        fea = torch.cat([self.expert[i](emb).unsqueeze(1)
                        for i in range(self.expert_num)], dim=1)
        task_fea = [torch.bmm(expert_gate_value[i], fea).squeeze(1)
                    for i in range(self.task_num)]

        critic_gate_value = [self.critic_gate(
            gate_input_emb[i]) for i in range(self.task_num)]
        results = []
        for i in range(self.task_num):
            output = [torch.sigmoid(self.critic[j](task_fea[i]))
...
class MetaHeacModel(torch.nn.Module):
    def __init__(self, categorical_field_dims, numerical_num, task_num,
                 ...
                 expert_num, critic_num, dropout
                 ,*args, **kwargs):
        super().__init__()
        self.model = HeacModel(categorical_field_dims, numerical_num, embed_dim,
                               bottom_mlp_dims, tower_mlp_dims, task_num, expert_num, critic_num, dropout)
    ...
    def local_update(self, support_set_categorical, support_set_numerical, support_set_y):
        ...
        for weight in fast_parameters:
            weight.fast = None
        support_set_y_pred = self.model(
            support_set_categorical, support_set_numerical)
        loss_list = [self.criterion(support_set_y_pred[j], support_set_y[:, j].float(
        )) for j in range(support_set_y.size(1))]
        loss = 0
        ...
        for k, weight in enumerate(self.model.parameters()):
            ...
            if weight.fast is None:
                weight.fast = weight - self.local_lr * \
    ...
    def global_update(self, list_sup_categorical, list_sup_numerical, list_sup_y, list_qry_categorical, list_qry_numerical, list_qry_y):
        ...
        for i in range(batch_sz):
            loss_sup = self.local_update(
                list_sup_categorical[i], list_sup_numerical[i], list_sup_y[i])
            query_set_y_pred = self.model(
                list_qry_categorical[i], list_qry_numerical[i])

            loss_list = [self.criterion(query_set_y_pred[j], list_qry_y[i][:, j].float(
            )) for j in range(list_qry_y[i].size(1))]
            loss = 0
            for item in loss_list:
                loss += item
            loss /= len(loss_list)
