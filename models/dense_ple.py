    ...
    def __init__(self, categorical_field_dims, numerical_num, task_num,
        ...
        for i in range(self.layers_num):
            ...
            self.share_experts[i] = torch.nn.ModuleList([MultiLayerPerceptron(input_dim, [
                                                        bottom_mlp_dims[i]], dropout, output_layer=False) for k in range(self.shared_expert_num)])
            ...
            for j in range(task_num):
                self.task_experts[i][j] = torch.nn.ModuleList([MultiLayerPerceptron(input_dim, [
                                                              bottom_mlp_dims[i]], dropout, output_layer=False) for k in range(self.specific_expert_num)])
    ...
    def forward(self, categorical_x, numerical_x):
        ...
        for i in range(self.layers_num):
            ...
            for j in range(self.task_num):
                ...
                task_fea[j] = torch.bmm(gate_value, mix_ouput).squeeze(1) + task_fea[j]
            ...
            if i != self.layers_num-1:  # not last layer
                ...
                task_fea[-1] = torch.bmm(gate_value, mix_ouput).squeeze(1) + task_fea[-1]
