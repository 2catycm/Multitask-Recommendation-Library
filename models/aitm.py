...
class AITMModel(MultitaskModel):
    ...
    def __init__(self, categorical_field_dims, numerical_num, task_num,
        ...
        self.g = torch.nn.ModuleList([torch.nn.Linear(
            bottom_mlp_dims[-1], bottom_mlp_dims[-1]) for i in range(task_num - 1)])
    ...
    def forward(self, categorical_x, numerical_x):
        ...
        for i in range(1, self.task_num):
            ...
            K = self.h2(x)
            Q = self.h3(x)
            fea[i] = torch.sum(torch.nn.functional.softmax(
                torch.sum(K * Q, 2, True) / np.sqrt(self.hidden_dim), dim=1) * V, 1)

        results = [torch.sigmoid(self.tower[i](fea[i]).squeeze(1))
